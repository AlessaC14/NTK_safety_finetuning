...
class KernelBehaviorDiagnostics:
    ...
    def __init__(self, model, tokenizer, label_token_ids, device='cuda'):
        self.model = model
        self.tokenizer = tokenizer
        self.label_token_ids = torch.tensor(label_token_ids, device=device)  # Convert to tensor on device
        self.device = device
    ...
    def measure_output_derivative(self, dataset: Dataset):
        ...
        with torch.no_grad():
            for ex in tqdm(dataset, desc="Measuring χ"):
                ...
                logits = outputs.logits[0, mask_pos, self.label_token_ids]  # Now label_token_ids is already on device
...
def get_pretrained_outputs(model, dataset, label_token_ids, device='cuda'):
    ...
    # Convert label_token_ids to tensor on device
    label_token_ids_tensor = torch.tensor(label_token_ids, device=device)
    ...
    with torch.no_grad():
        for ex in tqdm(dataset, desc="Pre-trained outputs"):
            ...
            logits = outputs.logits
            label_logits = logits[0, mask_pos, label_token_ids_tensor].cpu().numpy()
...
def main():
    ...
    CACHE_DIR = "/home/acarbol1/scratchenalisn1/acarbol1/NTK_safety_finetuning/kernel_cache_mnli"
    ...
    # Load model ONCE and move it to the correct device
    ...
    # Initialize diagnostics with the model that is already on the correct device
    ...
    test_pred_kernel, best_reg, best_f0 = regressor.fit_and_predict(
        np.array(train_labels), train_pretrain,
        test_train_kernel, test_pretrain,
        val_train_kernel, np.array(val_labels), val_pretrain
    )
